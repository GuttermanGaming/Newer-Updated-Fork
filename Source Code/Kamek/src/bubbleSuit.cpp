/* this is... not good.
TODO LIST:
* bubbles seem to move forever in "bursts"
* they seem to choose different angles to travel
* moves in a small circle after collecting an enemy 
* can only "throw" in 2 bursts, after the 2 bursts more can be thrown with an abolute max of 6
* moves faster than the player after spawning
* enemies don't interact with the bubble after it has collected a coin
* can't find anything that interacts with mario yet is ignored by the bubble, most things that make sense to get popped are popped */
...
void dSuitBubble_c::spriteCollision(ActivePhysics *apThis, ActivePhysics *apOther) {
	if (!this->hasTrappedEnemy && this->canBePopped) {
		...
		if (name != ProfileId::BubbleSuit) {OSReport("Bubble is colliding with %d\n", name);}
		...
		name == ProfileId::EN_RAIL_POLY_ICE || name == ProfileId::LIFT_ZEN_TOGE || name == ProfileId:: EN_HANACHAN || name == ProfileId::AC_PROP_BLOCK)
    	{if (name != ProfileId::BubbleSuit) {OSReport("Bubble is colliding with %d, so we're ignoring it\n", name);} return; }
		// list of actors that pop
		else if (name == ProfileId::EN_CHOROBON || name == ProfileId::WM_TREASURESHIP || name == ProfileId::EN_BOSS_KOOPA || name == ProfileId::EN_IGA_KURIBO || name == ProfileId::EN_UNIZOO || name == ProfileId::EN_SANBO || name == ProfileId::EN_BOSS_LARRY || 
		...
		name == ProfileId::EN_IBARAMUSHI || name == ProfileId::WM_SMALLCLOUD || name == ProfileId::AC_LIFT_ICE_SPRING)
...
int dSuitBubble_c::onCreate() {
    ...
    playerID = getNybbleValue(this->settings, 6, 6);
...
int dSuitBubble_c::onExecute() {
	...
	if (this->hasTrappedEnemy && !this->hasCoin) {
		coin = CreateActor(ProfileId::BubbleCoin, 0x3, this->pos, 0, 0);
		this->hasCoin = true;
